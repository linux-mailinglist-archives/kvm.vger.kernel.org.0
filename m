...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 92436140FFE
	for <lists+kvm@lfdr.de>; Fri, 17 Jan 2020 18:38:08 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727519AbgAQRiH (ORCPT <rfc822;lists+kvm@lfdr.de>);
...
+TEST_GEN_PROGS_x86_64 +=3D x86_64/svm_test
...
+	u16 val;
+
+	asm volatile ("mov %%cs, %0" : "=3Dmr"(val));
+	return val;
+}
+
...
+	u16 val;
+
+	asm volatile ("mov %%ds, %0" : "=3Dmr"(val));
+	return val;
+}
+
...
+	u16 val;
+
+	asm volatile ("mov %%es, %0" : "=3Dmr"(val));
+	return val;
+}
+
...
+	unsigned int val;
+
+	asm volatile ("mov %%ss, %0" : "=3Dmr"(val));
+	return val;
+}
+
+void generic_svm_setup(struct svm_test_data *svm, void *guest_rip, void =
...
+	asm volatile ("mov %%dr7, %0" : "=3Dr"(save->dr7));
+	asm volatile ("mov %%dr6, %0" : "=3Dr"(save->dr6));
...
+
+	vmcb->save.rip =3D (ulong)guest_rip;
+	vmcb->save.rsp =3D (ulong)guest_rsp;
+	regs.rdi =3D (ulong)svm;
+}
+
+#define SAVE_GPR_C                              \
+	"xchg %%rbx, regs+0x8\n\t"              \
...
+}
+
diff --git a/tools/testing/selftests/kvm/x86_64/svm_test.c b/tools/testin=
g/selftests/kvm/x86_64/svm_test.c
new file mode 100644
...
+{
+	asm volatile ("mov %%cr3, %0" : "=3Dr"(svm->test->scratch) : : "memory"=
);
+}
+static void prepare_cr3_intercept(struct svm_test_data *svm)
+{
+	svm->vmcb->control.intercept_cr_read |=3D 1 << 3;
+}
+
+static struct test tests[] =3D {
+	/* name, supported, custom setup, l2 code, exit code, custom check, fin=
ished */
+	{"vmmcall", NULL, NULL, l2_vmcall, SVM_EXIT_VMMCALL},
+	{"vmrun", NULL, NULL, l2_vmrun, SVM_EXIT_VMRUN},
+	{"CR3 read intercept", NULL, prepare_cr3_intercept, l2_cr3_read, SVM_EX=
IT_READ_CR3},
+};
+
...
+
+	GUEST_ASSERT(vmcb->control.exit_code =3D=3D
...
+		svm =3D vcpu_alloc_svm(vm, &svm_gva);
+		svm->test =3D &tests[i];
