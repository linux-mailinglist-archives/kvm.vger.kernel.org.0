Return-Path: <kvm+bounces-897-lists+kvm=lfdr.de@vger.kernel.org>
X-Original-To: lists+kvm@lfdr.de
Delivered-To: lists+kvm@lfdr.de
Received: from sv.mirrors.kernel.org (sv.mirrors.kernel.org [139.178.88.99])
	by mail.lfdr.de (Postfix) with ESMTPS id 772DB7E4238
	for <lists+kvm@lfdr.de>; Tue,  7 Nov 2023 15:58:11 +0100 (CET)
Received: from smtp.subspace.kernel.org (wormhole.subspace.kernel.org [52.25.139.140])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by sv.mirrors.kernel.org (Postfix) with ESMTPS id 265EB2811A4
	for <lists+kvm@lfdr.de>; Tue,  7 Nov 2023 14:58:10 +0000 (UTC)
Received: from localhost.localdomain (localhost.localdomain [127.0.0.1])
	by smtp.subspace.kernel.org (Postfix) with ESMTP id 10A9A31A6E;
	Tue,  7 Nov 2023 14:57:52 +0000 (UTC)
Authentication-Results: smtp.subspace.kernel.org;
	dkim=pass (2048-bit key) header.d=intel.com header.i=@intel.com header.b="daGodbQ7"
X-Original-To: kvm@vger.kernel.org
Received: from lindbergh.monkeyblade.net (lindbergh.monkeyblade.net [23.128.96.19])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-GCM-SHA384 (256/256 bits))
	(No client certificate requested)
	by smtp.subspace.kernel.org (Postfix) with ESMTPS id 2555630D04
	for <kvm@vger.kernel.org>; Tue,  7 Nov 2023 14:57:45 +0000 (UTC)
Received: from mgamail.intel.com (mgamail.intel.com [134.134.136.126])
	by lindbergh.monkeyblade.net (Postfix) with ESMTPS id 4B1E411C;
	Tue,  7 Nov 2023 06:57:41 -0800 (PST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple;
  d=intel.com; i=@intel.com; q=dns/txt; s=Intel;
  t=1699369061; x=1730905061;
  h=from:to:cc:subject:date:message-id:in-reply-to:
   references:mime-version:content-transfer-encoding;
  bh=VJ8fkdnjUgIDnH9XoCJGInUz6CqmlwZKqZuamoUHBSE=;
  b=daGodbQ7wvtEPatdDNC+w85obUGDUIEdbLac0zxXAtU2Pl0VovaG/g+4
   IGFqpjBFzZlmKNHaG2CGHKQ6DDX0BUtBq7sp70I9zQ8199TmgZKMuSA0P
   EtpIYywUyzlPLtK1KpT3ie+g/7HJ0ODOkNH9HB1srwpwNGeubZPY/8YD5
   O5Tbc4c9EmQk1Q1akb5mWryI6yeJ6e5u6Dbfd5KloGsaTovru6UkN7XUg
   B/5mGXZhs9EzdpX5k0kyxj548twAbQrQ/N/eoeS7o4y9dGi5ELEmwkPgq
   6EzmTTgnNnhyAD94U6kpR1Wb+VcmXJsP6DVRYwrPylGE0eLt9jlTlwjuw
   Q==;
X-IronPort-AV: E=McAfee;i="6600,9927,10887"; a="374555629"
X-IronPort-AV: E=Sophos;i="6.03,284,1694761200"; 
   d="scan'208";a="374555629"
Received: from orsmga004.jf.intel.com ([10.7.209.38])
  by orsmga106.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 07 Nov 2023 06:57:38 -0800
X-ExtLoop1: 1
X-IronPort-AV: E=McAfee;i="6600,9927,10887"; a="886319167"
X-IronPort-AV: E=Sophos;i="6.03,284,1694761200"; 
   d="scan'208";a="886319167"
Received: from ls.sc.intel.com (HELO localhost) ([172.25.112.31])
  by orsmga004-auth.jf.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 07 Nov 2023 06:57:34 -0800
...
Subject: [PATCH v17 001/116] KVM: VMX: Move out vmx_x86_ops to 'main.c' to wrap VMX and TDX
Date: Tue,  7 Nov 2023 06:55:27 -0800
Message-Id: <167bc6ff99c28ace90f7e9873734a3b5502eddda.1699368322.git.isaku.yamahata@intel.com>
...
index 000000000000..ea6c368db8cb
--- /dev/null
...
index 5ff90745e929..0e081c964e7a 100644
...
@@ -1657,7 +1655,7 @@ static int vmx_rtit_ctl_check(struct kvm_vcpu *vcpu, u64 data)
 	return 0;
 }
...
@@ -7751,7 +7745,7 @@ static void update_intel_pt_cfg(struct kvm_vcpu *vcpu)
...
@@ -7904,7 +7898,7 @@ static __init void vmx_set_cpu_caps(void)
...
@@ -7943,10 +7937,10 @@ static int vmx_check_intercept_io(struct kvm_vcpu *vcpu,
...
@@ -8026,8 +8020,8 @@ static inline int u64_shl_div_u64(u64 a, unsigned int shift,
 	return 0;
 }
...
@@ -8066,13 +8060,13 @@ static int vmx_set_hv_timer(struct kvm_vcpu *vcpu, u64 guest_deadline_tsc,
 	return 0;
 }
...
@@ -8101,7 +8095,7 @@ void vmx_update_cpu_dirty_logging(struct kvm_vcpu *vcpu)
...
@@ -8112,7 +8106,7 @@ static void vmx_setup_mce(struct kvm_vcpu *vcpu)
 }
...
+int vmx_smi_allowed(struct kvm_vcpu *vcpu, bool for_injection)
 {
 	...
 	if (to_vmx(vcpu)->nested.nested_run_pending)
@@ -8120,7 +8114,7 @@ static int vmx_smi_allowed(struct kvm_vcpu *vcpu, bool for_injection)
 	return !is_smm(vcpu);
 }
...
@@ -8141,7 +8135,7 @@ static int vmx_enter_smm(struct kvm_vcpu *vcpu, union kvm_smram *smram)
 	return 0;
 }
...
+int vmx_leave_smm(struct kvm_vcpu *vcpu, const union kvm_smram *smram)
 {
 	struct vcpu_vmx *vmx = to_vmx(vcpu);
 	int ret;
@@ -8162,18 +8156,18 @@ static int vmx_leave_smm(struct kvm_vcpu *vcpu, const union kvm_smram *smram)
 	return 0;
 }
...
@@ -8183,7 +8177,7 @@ static void vmx_migrate_timers(struct kvm_vcpu *vcpu)
 	}
 }
...
@@ -8193,167 +8187,13 @@ static void vmx_hardware_unsetup(void)
 	free_kvm_area();
 }
...
@@ -8419,9 +8259,7 @@ static void __init vmx_setup_me_spte_mask(void)
...
+__init int vmx_hardware_setup(void)
 {
 	...
 	struct desc_ptr dt;
@@ -8490,16 +8328,16 @@ static __init int hardware_setup(void)
...
@@ -8514,7 +8352,7 @@ static __init int hardware_setup(void)
...
+		vt_x86_ops.sync_pir_to_irr = NULL;
 	...
 	if (!enable_apicv || !cpu_has_vmx_ipiv())
 		enable_ipiv = false;
@@ -8550,7 +8388,7 @@ static __init int hardware_setup(void)
 		enable_pml = 0;
...
@@ -8575,9 +8413,9 @@ static __init int hardware_setup(void)
 	}
...
@@ -8588,9 +8426,9 @@ static __init int hardware_setup(void)
...
@@ -8613,14 +8451,6 @@ static __init int hardware_setup(void)
 	return r;
 }
...
@@ -8662,7 +8492,7 @@ static int __init vmx_init(void)
 	 */
 	hv_init_evmcs();
...
new file mode 100644
index 000000000000..4af1b01aa9c0
--- /dev/null
