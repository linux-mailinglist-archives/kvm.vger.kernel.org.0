...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 2040213DA46
	for <lists+kvm@lfdr.de>; Thu, 16 Jan 2020 13:44:59 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1726950AbgAPMnZ (ORCPT <rfc822;lists+kvm@lfdr.de>);
        Thu, 16 Jan 2020 07:43:25 -0500
Received: from us-smtp-delivery-1.mimecast.com ([207.211.31.120]:32205 "EHLO
        us-smtp-1.mimecast.com" rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org
        with ESMTP id S1726574AbgAPMnZ (ORCPT <rfc822;kvm@vger.kernel.org>);
        Thu, 16 Jan 2020 07:43:25 -0500
...
+	int ret;
+
+	ret =3D iotlb_translate(vrh, (u64)src, len, iov, 16, VHOST_MAP_RO);
+	if (ret < 0)
+		return ret;
+
...
+	int ret;
+
+	ret =3D iotlb_translate(vrh, (u64)dst, len, iov, 16, VHOST_MAP_WO);
+	if (ret < 0)
+		return ret;
+
...
+	ret =3D iotlb_translate(vrh, (u64)p, sizeof(*p),
+			      &iov, 1, VHOST_MAP_RO);
+	if (ret < 0)
+		return ret;
+
...
+	}
+
+	vrh->little_endian =3D (features & (1ULL << VIRTIO_F_VERSION_1));
+	vrh->event_indices =3D (features & (1 << VIRTIO_RING_F_EVENT_IDX));
+	vrh->weak_barriers =3D weak_barriers;
+	vrh->completed =3D 0;
+	vrh->last_avail_idx =3D 0;
+	vrh->last_used_idx =3D 0;
+	vrh->vring.num =3D num;
+	vrh->vring.desc =3D desc;
+	vrh->vring.avail =3D avail;
+	vrh->vring.used =3D used;
+	return 0;
+}
