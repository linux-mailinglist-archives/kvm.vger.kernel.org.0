...
Received: from vger.kernel.org (vger.kernel.org [23.128.96.18])
	by mail.lfdr.de (Postfix) with ESMTP id 89E2C3C202D
	for <lists+kvm@lfdr.de>; Fri,  9 Jul 2021 09:48:54 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231327AbhGIHvg (ORCPT <rfc822;lists+kvm@lfdr.de>);
        Fri, 9 Jul 2021 03:51:36 -0400
Received: from mga01.intel.com ([192.55.52.88]:51178 "EHLO mga01.intel.com"
        rhost-flags-OK-OK-OK-OK) by vger.kernel.org with ESMTP
        id S231233AbhGIHvf (ORCPT <rfc822;kvm@vger.kernel.org>);
        Fri, 9 Jul 2021 03:51:35 -0400
X-IronPort-AV: E=McAfee;i="6200,9189,10039"; a="231431262"
X-IronPort-AV: E=Sophos;i="5.84,226,1620716400"; 
   d="scan'208";a="231431262"
Received: from orsmga003.jf.intel.com ([10.7.209.27])
  by fmsmga101.fm.intel.com with ESMTP/TLS/ECDHE-RSA-AES256-GCM-SHA384; 09 Jul 2021 00:48:51 -0700
X-ExtLoop1: 1
X-IronPort-AV: E=Sophos;i="5.84,226,1620716400"; 
   d="scan'208";a="411707954"
...
x-forefront-antispam-report: CIP:255.255.255.255;CTRY:;LANG:en;SCL:1;SRV:;IPV:NLI;SFV:NSPM;H:BN9PR11MB5433.namprd11.prod.outlook.com;PTR:;CAT:NONE;SFS:(346002)(396003)(366004)(136003)(39860400002)(376002)(66476007)(4326008)(7416002)(8676002)(66556008)(45080400002)(5660300002)(66946007)(966005)(83380400001)(2906002)(30864003)(478600001)(64756008)(66446008)(76116006)(7696005)(86362001)(33656002)(316002)(8936002)(9686003)(52536014)(921005)(38100700002)(54906003)(55016002)(122000001)(6506007)(110136005)(186003)(26005)(71200400001)(559001)(579004);DIR:OUT;SFP:1102;
...
X-MS-Exchange-CrossTenant-originalarrivaltime: 09 Jul 2021 07:48:44.2991
 (UTC)
...
/dev/iommu provides an unified interface for managing I/O page tables for=20
...
This proposal describes the uAPI of /dev/iommu and also sample sequences=20
...
IOMMU fd is the container holding multiple I/O address spaces. User=20
...
An I/O address space takes effect only after it is attached by a device.=20
One device is allowed to attach to multiple I/O address spaces. One I/O=20
...
The format of an I/O page table must be compatible to the attached=20
...
will be attached right after. Attaching a device to an IOASID with=20
incompatible format is simply rejected.

Relationship between IOMMU fd, VFIO fd and KVM fd:

-   IOMMU fd provides uAPI for managing IOASIDs and I/O page tables.=20
    It also provides an unified capability/format reporting interface for
    each bound device.=20

-   VFIO fd provides uAPI for device binding and attaching. In this proposa=
l=20
...
-   kernel-managed I/O page table which is created via IOMMU fd, e.g.=20
    for IOVA space (dpdk), GPA space (Qemu), GIOVA space (vIOMMU), etc.

-   user-managed I/O page table which is created by the user, e.g. for=20
    GIOVA/GVA space (vIOMMU), etc.

-   shared kernel-managed CPU page table which is created by another=20
    subsystem, e.g. for process VA space (mm), GPA space (kvm), etc.

The first category is managed via a dma mapping protocol (similar to=20
existing VFIO iommu type1), which allows the user to explicitly specify=20
...
In the meantime, a fault reporting/completion mechanism is also provided=20
for the user to fixup potential I/O page faults.

The last category is supposed to be managed via the subsystem which
actually owns the shared address space. Likely what's minimally required=20
in /dev/iommu uAPI is to build the connection with the address space=20
owner when allocating the IOASID, so an in-kernel interface (e.g. mmu_
notifer) is activated for any required synchronization between IOMMU fd=20
and the space owner.

This proposal focuses on how to manage the first two categories, as=20
they are existing and more urgent requirements. Support of the last
category can be discussed when a real usage comes in the future.=20

The user needs to specify the desired management protocol and page=20
table format when creating a new I/O address space. Before allocating=20
the IOASID, the user should already know at least one device that will be=20
...
attached device (or a common set between multiple devices) and then=20
choose a compatible format to set on the IOASID.

I/O address spaces can be nested together, called IOASID nesting. IOASID
nesting can be implemented in two ways: hardware nesting and software=20
nesting. With hardware support the child and parent I/O page tables are=20
walked consecutively by the IOMMU to form a nested translation. When=20
it's implemented in software, /dev/iommu is responsible for merging the=20
two-level mappings into a single-level shadow I/O page table.=20

An user-managed I/O page table can be setup only on the child IOASID,=20
implying IOASID nesting must be enabled. This is because the kernel=20
doesn't trust userspace. Nesting allows the kernel to enforce its DMA=20
isolation policy through the parent IOASID.=20

Software nesting is useful in several scenarios. First, it allows=20
centralized accounting on locked pages between multiple root IOASIDs
(no parent). In this case a 'dummy' IOASID can be created with an=20
identity mapping (HVA->HVA), dedicated for page pinning/accounting and=20
nested by all root IOASIDs. Second, it's also useful for mdev drivers=20
(e.g. kvmgt) to write-protect guest structures when vIOMMU is enabled.=20
In this case the protected addresses are in GIOVA space while KVM=20
write-protection API is based on GPA. Software nesting allows finding=20
...
Successful binding puts the device into a security context which isolates=20
...
specific routing information for where to install the I/O page table in=20
...
Requester ID (RID) identifying the device plus optional Process Address=20
Space ID (PASID). For ARM, it is Stream ID (SID) plus optional Sub-Stream=20
...
on a vfio_device based on the hardware capability. In such case the user=20
has its own view of the virtual routing information (vPASID) when marking=20
...
explicitly register the vPASID information to the kernel when attaching=20
the vfio_device. In this case VFIO must figure out whether vPASID should=20
be directly used (pdev) or converted to a kernel-allocated pPASID (mdev)=20
for physical routing. Detail explanation about PASID virtualization can=20
be found in {1.4}.
...
defPASID) when talking about the default I/O address space on mdev. When=20
vPASID or pPASID is referred in PASID virtualization it's all about the=20
non-default spaces. defPASID and pPASID are always hidden from userspace=20
and can only be indirectly referenced via IOASID.

1.3. Group isolation
++++++++++++++++++++

Group is the minimal object when talking about DMA isolation in the=20
iommu layer. Devices which cannot be isolated from each other are=20
organized into a single group. Lack of isolation could be caused by=20
multiple reasons: no ACS capability in the upstreaming port, behind a=20
PCIe-to-PCI bridge (thus sharing RID), or DMA aliasing (multiple RIDs=20
per device), etc.

All devices in the group must be put in a security context together=20
before one or more devices in the group are operated by an untrusted=20
user. Passthrough frameworks must guarantee that:

1)  No user access is granted on a device before an security context is=20
...
=20
    user, or driver-less, or bound to a driver which is known safe (not=20
    do DMA).=20
...
1)  VFIO user can open a device fd only after:
    ...
    * Group viability is passed and the entire group is attached to=20
...
3)  Detach the group from the container when the last device fd in the=20
    group is closed and destroy the I/O page table only after the last=20
    group is detached from the container.

With this proposal VFIO can move to a simpler device-centric model by
directly exposeing device node under "/dev/vfio/devices" w/o using=20
container and group uAPI at all. In this case group isolation is enforced
mplicitly within IOMMU fd:

1)  A successful binding call for the first device in the group creates=20
...
2)  IOMMU fd monitors driver binding status in case group viability is
    broken, same as VFIO does today. BUG_ON() might be eliminated if we=20
    ...
    Switch between two domains is initiated by attaching the device to or=20
    detaching it from an IOASID. The IOMMU layer should ensure that=20
    ...
    To stay on par with legacy VFIO, IOMMU fd could verify that all=20
...
Actual implementation may use a staging approach, e.g. only support=20
...
If necessary, devices within a group may be further allowed to be=20
attached to different IOASIDs in the same IOMMU fd, in case that the=20
source devices can be reliably identifiable (e.g. due to !ACS). This will=20
require additional sub-group logic in the iommu layer and with=20
sub-group topology exposed to userspace. But no expectation of=20
...
https://lore.kernel.org/linux-iommu/BN9PR11MB543382665D34E58155A9593C8C039@=
...
address spaces are supported on a device. The actual policy is per-device=20
thus defined by specific VFIO device driver.=20

A PASID virtualization policy is defined by four aspects:

1)  Whether this device allows the user to create multiple I/O address=20
    spaces (vPASID capability). This is decided upon whether this device=20
...
3)  If no, the user should register vPASID to the kernel. Then the next
    ...
    (vPASID=3D=3DpPASID or vPASID!=3DpPASID). The key is whether this devic=
e=20
    must share the PASID space with others (pdev vs. mdev).

4)  If vPASID!=3DpPASID, whether pPASID should be allocated from the=20
    per-RID space or a global space. This is about whether the device=20
    supports PCIe DMWr-type work submission (e.g. Intel ENQCMD) which=20
...
vPASID!=3DpPASID, new pPASIDs will be re-allocated on the destination and
VFIO device driver is responsible for programming the device to use the
new pPASID when restoring the device state.

Different policies may imply different uAPI semantics for user to follow=20
when attaching a device. The semantics information is expected to be=20
reported to the user via VFIO uAPI instead of via IOMMU fd, since the=20
latter only cares about pPASID. But if there is a different thought we'd=20
like to hear it.

Following sections (1.4.1 - 1.4.3) provide detail explanation on how=20
above are selected on different device types and the implication when=20
multiple types are mixed together (i.e. assigned to a single user). Last=20
...
6)  a mdev which exposes vPASID and shares the parent's PASID table
  ...
  +--------+---------+---------+----------+-----------+
  |        |         |Delegated| vPASID=3D=3D |  per-RID  |
...
=3D=3D=3D+=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D+=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D+
  ...
  +--------+---------+---------+----------+-----------+
  | type-4 |    Yes  |   Yes   |   v=3D=3Dp(*)| per-RID(*)|
  +--------+---------+---------+----------+-----------+
  | type-5 |    Yes  |   No    |   v=3D=3Dp   |  per-RID  |
  +--------+---------+---------+----------+-----------+
  | type-6 |    Yes  |   No    |   v!=3Dp   |  per-RID  |
  +--------+---------+---------+----------+-----------+
  <* conceptual definition though the PASID space is fully delegated>

for 1-3 there is no vPASID capability exposed and the user can create=20
only one default I/O address space on this device. Thus there is no PASID=20
virtualization at all.

4) is specific to ARM/AMD platforms where the PASID table is managed by=20
the user. In this case the entire PASID space is delegated to the user
which just needs to create a single IOASID linked to the user-managed=20
PASID table, as placeholder covering all non-default I/O address spaces=20
on pdev. In concept this looks like a big 84bit address space (20bit=20
PASID + 64bit addr). vPASID may be carried in the uAPI data to help define=
=20
the operation scope when invalidating IOTLB or reporting I/O page fault.=20
IOMMU fd doesn't touch it and just acts as a channel for vIOMMU/pIOMMU to=20
exchange info.

5) is specific to Intel platforms where the PASID table is managed by=20
the kernel. In this case vPASIDs should be registered to the kernel=20
in the attaching call. This implies that every non-default I/O address=20
space on pdev is explicitly tracked by an unique IOASID in the kernel.=20
Because pdev is fully controlled by the user, its DMA request carries=20
vPASID as the routing informaiton thus requires VFIO device driver to=20
adopt vPASID=3D=3DpPASID policy. Because an IOASID already represents a
standalone address space, there is no need to further carry vPASID in=20
the invalidation and fault paths.

6) is about mdev, as those enabled by Intel Scalable IOV. The main=20
difference from type-5) is on whether vPASID=3D=3DpPASID. There is=20
only a single PASID table per the parent device, implying the per-RID=20
PASID space shared by all mdevs created on this parent. VFIO device=20
driver must use vPASID!=3DpPASID policy and allocate a pPASID from the=20
per-RID space for every registered vPASID to guarantee DMA isolation=20
...
-   When VFIO device driver updates a PASID MMIO register in the=20
...
based on PCI Deferrable Memory Write (DMWr) capability, allowing a=20
single work queue to access multiple I/O address spaces. One example=20
using DMWr is Intel ENQCMD, having PASID saved in the CPU MSR and=20
carried in the non-posted DMWr payload when sent out to the device.=20
Then a single work queue shared by multiple processes can compose=20
DMAs toward different address spaces, by carrying the PASID value=20
retrieved from the DMWr payload. The role of DMWr is allowing the=20
shared work queue to return a retry response when the work queue
is under pressure (due to capacity or QoS). Upon such response the=20
software could try re-submitting the descriptor.

When ENQCMD is executed in the guest, the value saved in the CPU=20
MSR is vPASID (part of the xsave state). This creates another point for=20
...
and respective polices:
  ...
  +=3D=3D=3D=3D=3D=3D=3D=3D+=3D=3D=3D=3D=3D=3D=3D=3D=3D+=3D=3D=3D=3D=3D=3D=
=3D=3D=3D+=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D+=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D=3D+
  | type-7 |    Yes  |   Yes   |   v=3D=3Dp   |  per-RID  |
  +--------+---------+---------+----------+-----------+
  | type-8 |    Yes  |   Yes   |   v!=3Dp   |  global   |
  +--------+---------+---------+----------+-----------+

DMWr or shared mode is configurable per work queue. It's completely=20
sane if an assigned device with multiple queues needs to handle both=20
DMWr (shared work queue) and normal write (dedicated work queue)=20
simultaneously. Thus the PASID virtualization policy must be consistent=20
when both paths are activated.

for 7) we should use the same policy as 5), i.e. directly using vPASID=20
for physical routing on pdev. In this case ENQCMD in the guest just works=20
w/o additional work because the vPASID saved in the PASID_MSR=20
matches the routing information configured for the target I/O address
space in the IOMMU. When receiving a DMWr request, the shared=20
work queue grabs vPASID from the payload and then tags outgoing=20
...
for 8) vPASID in the PASID_MSR must be converted to pPASID before=20
sent to the wire (given vPASID!=3DpPASID for the same reason as 6).=20
Intel CPU provides a hardware PASID translation capability for auto-
conversion when ENQCMD is being executed. In this case the payload=20
received by the work queue contains pPASID thus outgoing DMAs are=20
tagged with pPASID. This is consistent with the dedicated work=20
queue path where pPASID is programmed to the MMIO register in the=20
mediation path and then grabbed to tag DMAs.

However, the CPU translation structure is per-VM which implies
that a same pPASID must be used cross all type-8 devices (of this VM)=20
...
CPU translation capability is enabled via KVM uAPI. We need a secure=20
contract between VFIO device fd and KVM fd so VFIO device driver knows=20
when it's secure to allow guest access to the cmd portal of the type-8
device. It's dangerous by allowing the guest to issue ENQCMD to the=20
device before CPU is ready for PASID translation. In this window the=20
vPASID is untranslated thus grants the guest to access random I/O=20
address space on the parent of this mdev.

We plan to utilize existing kvm-vfio contract. It is currently used for=20
...
translation capability is turned on. Before receiving this notification,=20
the VFIO device driver should not allow user to access the DMWr-capable=20
work queue on type-8 device.

1.4.3. Mix different types together
***********************************

In majority case mixing different types doesn't change the aforementioned=20
PASID virtualization policy for each type. The user just needs to handle=20
them per device basis.=20

There is one exception though, when mixing type 7) and 8) together,
due to conflicting policies on how PASID_MSR should be handled.=20
For mdev (type-8) the CPU translation capability must be enabled to=20
prevent a malicious guest from doing bad things. But once per-VM=20
PASID translation is enabled, the shared work queue of pdev (type-7)=20
will also receive a pPASID allocated for mdev instead of the vPASID=20
that is expected on this pdev.

Fixing this exception for pdev is not easy. There are three options.

One is moving pdev to also accept pPASID. Because pdev may have both=20
shared work queue (PASID in MSR) and dedicated work queue (PASID
in MMIO) enabled by the guest, this requires VFIO device driver to=20
mediate the dedicated work queue path so vPASIDs programmed by=20
the guest are manually translated to pPASIDs before written to the=20
pdev. This may add undesired software complexity and potential=20
performance impact if the PASID register locates alongside other=20
fast-path resources in the same 4K page. If it works it essentially=20
converts type-7 to type-8 from user p.o.v.

The second option is using an enlightened approach so the guest=20
...
the need of mediation. However this requires different uAPI semantics=20
(from register-vPASID to return-pPASID) and exposes pPASID knowledge=20
to userspace which also implies breaking VM live migration.

The third option is making pPASID as an alias routing info to vPASID=20
and having both linked to the same I/O page table in the IOMMU, so=20
either way can hit the desired address space. This further requires sort=20
of range split scheme to avoid conflict between vPASID and pPASID.=20
However, we haven't found a clear way to fold this trick into this uAPI=20
proposal yet. and this option may not work when PASID is also used to=20
tag the IMS entry for verifying the interrupt source. In this case there=20
is no room for aliasing.

So, none of above can work cleanly based on current thoughts. We=20
decide to not support type-7/8 mix in this proposal. User could detect=20
...
spaces with vPASID on the device. This capability further includes=20
...
The last two flags together help the user to detect the unsupported=20
type 7/8 mix scenario.

Take Qemu for example. It queries above flags for every vfio device at=20
initialization time, after identifying the PASID capability:

1)  If PASID_DELEGATED is set, the PASID space is fully managed by the=20
    user thus a single IOASID (linked to user-managed page table) is=20
    required as the placeholder for all non-default I/O address spaces=20
    on the device.

    If not set, an IOASID must be created for every non-default I/O address=
=20
    space on this device and vPASID must be registered to the kernel=20
    when attaching the device to this IOASID.

    User may want to sanity check on all devices with the same setting=20
...
2)  If PASID_CPU is not set, done.
    ...
    Otherwise check whether the PASID_CPU_VIRT flag on this device is=20
...
3)  If PASID_CPU_VIRT is not set, done.
    ...
    If set and consistency check in 2) is passed, call KVM uAPI to=20
    enable CPU PASID translation if it is the first device with this flag=20
...
Snoop behavior of a DMA specifies whether the access is coherent (snoops=20
the processor caches) or not. The snoop behavior is decided by both device=
=20
and IOMMU. Device can set a no-snoop attribute in DMA request to force=20
the non-coherent behavior, while IOMMU may support a configuration which=20
enforces DMAs to be coherent (with the no-snoop attribute ignored).

No-snoop DMA requires the driver to manually flush caches for=20
observing the latest content. When such driver is running in the guest,=20
it further requires KVM to intercept/emulate WBINVD plus favoring=20
guest cache attributes in the EPT page table.

Alex helped create a matrix as below:
(https://lore.kernel.org/linux-iommu/PH0PR12MB54811863B392C644E5365446DC3E9=
@PH0PR12MB5481.namprd12.prod.outlook.com/T/#mbfc96278b078d3ec07eabb9aa46abf=
e03a886dc6)
...
learn whether no-snoop DMA is used thus special tricks on WBINVD=20
and EPT must be enabled. However, the criteria of no-snoop DMA is=20
solely based on the fact of lacking IOMMU enforce-snoop for any vfio=20
device, i.e. both 3) and 4) are considered capable of doing no-snoop=20
...
-   Unnecessary overhead in KVM side in 4) or if the driver doesn't do=20
    no-snoop DMA in 3). Although the driver doesn't use WBINVD, the=20
...
control over the no-snoop usage. In this model the enforce-snoop format=20
is specified when an IOASID is created, while the device no-snoop usage=20
can be further clarified when it's attached to the IOASID.=20
...
-   allowing user to specify whether an IOASID should be created in the=20
    ...
    * Any device can be attached to a non-enforce-snoop IOASID,=20
    ...
    * Attaching a device which cannot be forced to snoop by its IOMMU=20
...
-   allowing user to further hint whether no-snoop DMA is actually used=20
    ...
    * {4} can be filtered out automatically by VFIO device driver,=20
      based on device no-snoop capability.

    * If no hint is provided, fall back to legacy VFIO policy, i.e.=20
...
-   a new contract for KVM to learn whether any IOASID is attached by
    ...
    * The new contract will allows KVM to query no-snoop requirement=20
      per IOMMU fd. This will apply to all passthrough frameworks.

    * A notification mechanism might be introduced to switch between
      WBINVD emulation and no-op intercept according to device=20
      attaching status change in registered IOMMU fd.

    * whether kvm-vfio will be completely deprecated is a TBD. It's=20
      still used for non-iommu related contract, e.g. notifying kvm=20
...
{1, 2}), while leaving {3, 4} still handled via legacy VFIO. In=20
this case no need to introduce new contract with KVM. An easy way is=20
having VFIO not expose {3, 4} devices in /dev/vfio/devices. Then we have=20
plenty of time to figure out the implementation detail of the new model=20
at a later stage.
...
/dev/vfio device uAPI builds connection between devices and I/O address=20
spaces.

/dev/kvm uAPI is optionally required as far as no-snoop DMA or ENQCMD=20
is concerned.
...
/*
  ...
  *
  * It's unlikely that all planned capabilities in IOMMU fd will be ready i=
n
  * one breath. User should check which uAPI extension is supported=20
...
/*
  ...
  *
  * It could be reported either via a capability chain as implemented in=20
  * VFIO or a per-capability query interface. The device is identified=20
...
is
  ...
  * which will be attached to this IOASID right after. The I/O page table=20
  ...
  *
  * The root IOASID should always have a kernel-managed I/O page=20
  ...
  * are created but IOASID nesting is disabled. However, one page might=20
  * be accounted multiple times in this case. The user is recommended to=20
  * instead create a 'dummy' root with identity mapping (HVA->HVA) for=20
  * centralized accounting, nested by all other IOASIDs which represent=20
...
/*
  ...
  * pinned and accounted as locked memory. Pinning might be postponed=20
  * until the IOASID is attached by a device. Software mdev driver may=20
  * further provide a hint to skip auto-pinning at attaching time, since
  * it does selective pinning at run-time. auto-pinning can be also=20
...
=20
  * with hardware nesting, as the IOMMU will catch up the change=20
...
/*
  ...
  * granularities. Device cookie and vPASID may be specified to help=20
...
/*
  ...
  * include a fault region to report fault data via read()), an=20
  * eventfd to notify the user and an ioctl to complete the fault.
  *
  * The fault data includes {IOASID, device_cookie, faulting addr, perm}=20
...
/*
  ...
  *
  * The user should provide a device cookie when calling this ioctl. The=20
  * cookie is later used in IOMMU fd for capability query, iotlb invalidati=
on
...
/*
  ...
  *
  *	- PASID_DELEGATED, if clear every vPASID must be registered to=20
  *	  the kernel;
  *	- PASID_CPU, if set vPASID is allowed to be carried in the CPU=20
  *	  instructions (e.g. ENQCMD);
  *	- PASID_CPU_VIRT, if set require vPASID translation in the CPU;=20
  *=20
  * The user must check that all devices with PASID_CPU set have the=20
  * same setting on PASID_CPU_VIRT. If mismatching, it should enable=20
  ...
  * to use ENQCMD to submit work items. The command portal is blocked=20
...
.=20
  * Whether the virtual PASID is physically used or converted to another=20
...
/*
  ...
  * this will be a device-centric interface, thus existing vfio-kvm contrac=
t
...
Later some group-tracking fields will be also introduced to support=20
multi-devices group.
...
For mdev the struct device should be the pointer to the parent device.=20

An ioasid_data is created when IOMMU_IOASID_ALLOC, as the main=20
...
iommu_domain is the object for operating the kernel-managed I/O=20
...
-   root IOASID (except the 'dummy' one for locked accounting)
    must use kernel-manage I/O page table thus always linked to an=20
    iommu_domain;
...
-   child IOASID (via hardware nesting) is linked to another simpler iommu
    layer object (TBD) for tracking user-managed page table. Due to=20
    nesting it is also implicitly linked to the iommu_domain of the=20
    parent;

Following link has an initial discussion on this part:

https://lore.kernel.org/linux-iommu/BN9PR11MB54331FC6BB31E8CBF11914A48C019@=
BN9PR11MB5433.namprd11.prod.outlook.com/T/#m2c19d3825cc096daf2026ea94e00cc5=
858cda321
...
explicitly to support VFIO_ATTACH_IOASID, e.g.
	...
	struct iommu_attach_data * iommu_pci_device_attach(
		struct iommu_dev *dev, struct pci_device *pdev,=20
		u32 ioasid);
...
=20
be acquired w/o going through legacy container/group interface. They=20
...
(e.g. in a group with multiple devices, or support no-snoop DMA). In concep=
t
there is no problem for VFIO to support two models simultaneously, but=20
...
:
	...
	iommu_fd =3D open("/dev/iommu", mode);
...
Examples here apply to both pdev and mdev. VFIO device driver in the=20
...
GET_INFO are skipped in these examples. No-snoop DMA is also not covered he=
re.

Below examples may not apply to all platforms. For example, the PAPR IOMMU
in PPC platform always requires a vIOMMU and blocks DMAs until the device i=
s=20
explicitly attached to an GIOVA address space. there are even fixed=20
associations between available GIOVA spaces and devices. Those platform=20
specific variances are not covered here and will be figured out in the=20
...
One gpa_ioasid is created. The GPA address space is managed through=20
...
If the guest is assigned with more than dev1, the user follows above=20
sequence to attach other devices to the same gpa_ioasid i.e. sharing=20
the GPA address space cross all assigned devices, e.g. for dev2:

	bind_data =3D {.fd =3D iommu_fd; .cookie =3D cookie2};
...
VFIO works today). The side-effect is that duplicated locked page=20
...
IOASIDs now. It will be fixed once IOASID nesting is supported:
	...
	/* Bind device to IOMMU fd */
	bind_data =3D {.fd =3D iommu_fd; .device_cookie =3D cookie1};
	ioctl(device_fd1, VFIO_BIND_IOASID_FD, &bind_data);
	bind_data =3D {.fd =3D iommu_fd; .device_cookie =3D cookie2};
	ioctl(device_fd2, VFIO_BIND_IOASID_FD, &bind_data);
...
Same usage scenario as 4.2, with software-based IOASID nesting=20
available. In this mode it is the kernel instead of user to create the
shadow mapping.

The flow before guest boots is same as 4.2, except one point. Because=20
...
to incompatible format between dev1/dev2 (e.g. about IOMMU=20
enforce-snoop). In such case the user could further created a dummy
IOASID (HVA->HVA) as the root parent for two gpa_ioasids to avoid=20
duplicated accounting. But this scenario is not covered in following=20
flows.
...
have been attached to gpa_ioasid before guest boots):
	...
	/* Setup a GIOVA [0x2000] ->GPA [0x1000] mapping for giova_ioasid,=20
...
After boots the guest further creates a GVA address spaces (vpasid1) on=20
...
/****** If dev1 reports PASID_DELEGATED=3Dfalse **********/
	/* After boots */
	...
	gva_ioasid =3D ioctl(iommu_fd, IOMMU_IOASID_ALLOC, &alloc_data);

	/* Attach dev1 to the new address space (child) and specify=20
	 * vPASID. Note dev1 is still attached to gpa_ioasid (parent)
	 */
	at_data =3D {
		.fd		=3D iommu_fd;
		.ioasid		=3D gva_ioasid;
		.flag 		=3D IOASID_ATTACH_VPASID;
		.vpasid		=3D vpasid1;
	};
	ioctl(device_fd1, VFIO_ATTACH_IOASID, &at_data);
...
/****** If dev1 reports PASID_DELEGATED=3Dtrue **********/
	...
	pasidtbl_ioasid =3D ioctl(iommu_fd, IOMMU_IOASID_ALLOC, &alloc_data);

	/* Attach dev1 to the vPASID space */
	at_data =3D {.fd =3D iommu_fd; .ioasid =3D pasidtbl_ioasid};
	ioctl(device_fd1, VFIO_ATTACH_IOASID, &at_data);

	/* from now on all GVA address spaces on dev1 are represented by=20
	 * a single pasidtlb_ioasid as the placeholder in the kernel.
	 *
	 * But iotlb invalidation and fault handling are still per GVA=20
	 * address space. They are still going through IOMMU fd in the=20
	 * same way as PASID_DELEGATED=3Dfalse scenario
	 */
	...
...
-   Host IOMMU driver receives a I/O page fault with raw fault_data {rid,=20
    pasid, addr};
...
-   IOASID fault handler identifies the corresponding ioasid and device=20
    cookie according to the opaque object, generates an user fault_data=20
    (ioasid, cookie, addr) in the fault region, and triggers eventfd to=20
    userspace;
...
=20
    (v_rid + v_pasid) of the device attached to the faulting ioasid;
      ...
      * v_pasid is either identified according to ioasid, or already carrie=
d
        in the fault data;
...
-   Qemu finds the pending fault event, converts virtual completion data=20
    into (ioasid, cookie, response_code), and then calls a /dev/iommu ioctl=
 to=20
