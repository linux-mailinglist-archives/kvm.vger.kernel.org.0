...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 089D41940F0
	for <lists+kvm@lfdr.de>; Thu, 26 Mar 2020 15:05:38 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1727892AbgCZOFE (ORCPT <rfc822;lists+kvm@lfdr.de>);
        Thu, 26 Mar 2020 10:05:04 -0400
Received: from us-smtp-delivery-74.mimecast.com ([63.128.21.74]:30292 "EHLO
        us-smtp-delivery-74.mimecast.com" rhost-flags-OK-OK-OK-OK)
        by vger.kernel.org with ESMTP id S1727822AbgCZOFB (ORCPT
        <rfc822;kvm@vger.kernel.org>); Thu, 26 Mar 2020 10:05:01 -0400
...
+	u32 i;
+
+	ret =3D pci_read_config_byte(pdev, PCI_CAPABILITY_LIST, &pos);
+	if (ret < 0) {
...
+
+	while (pos) {
+		ret =3D ifcvf_read_config_range(pdev, (u32 *)&cap,
+					      sizeof(cap), pos);
+		if (ret < 0) {
+			IFCVF_ERR(pdev,
...
+			IFCVF_DBG(pdev, "hw->common_cfg =3D %p\n",
+				  hw->common_cfg);
+			break;
...
+			IFCVF_DBG(pdev, "hw->notify_base =3D %p\n",
+				  hw->notify_base);
+			break;
+		case VIRTIO_PCI_CAP_ISR_CFG:
+			hw->isr =3D get_cap_addr(hw, &cap);
+			IFCVF_DBG(pdev, "hw->isr =3D %p\n", hw->isr);
+			break;
+		case VIRTIO_PCI_CAP_DEVICE_CFG:
+			hw->net_cfg =3D get_cap_addr(hw, &cap);
+			IFCVF_DBG(pdev, "hw->net_cfg =3D %p\n", hw->net_cfg);
+			break;
+		}
+
+next:
...
+	}
+
+	hw->lm_cfg =3D hw->base[IFCVF_LM_BAR];
+
+	IFCVF_DBG(pdev,
...
+#define IFCVF_INFO(pdev, fmt, ...)	dev_info(&pdev->dev, fmt, ##__VA_ARGS=
__)
+
...
+	}
+
+	ret =3D pcim_iomap_regions(pdev, BIT(0) | BIT(2) | BIT(4),
+				 IFCVF_DRIVER_NAME);
+	if (ret) {
...
+	}
+
+	ret =3D devm_add_action_or_reset(dev, ifcvf_free_irq_vectors, pdev);
+	if (ret) {
+		IFCVF_ERR(pdev,
...
+
+	vf =3D &adapter->vf;
+	vf->base =3D pcim_iomap_table(pdev);
+
