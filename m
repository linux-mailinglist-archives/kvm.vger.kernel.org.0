...
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id 04B181474D6
	for <lists+kvm@lfdr.de>; Fri, 24 Jan 2020 00:33:38 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1729509AbgAWXdh (ORCPT <rfc822;lists+kvm@lfdr.de>);
        Thu, 23 Jan 2020 18:33:37 -0500
Received: from us-smtp-delivery-1.mimecast.com ([207.211.31.120]:59620 "EHLO
        us-smtp-1.mimecast.com" rhost-flags-OK-OK-OK-FAIL) by vger.kernel.org
        with ESMTP id S1729347AbgAWXdg (ORCPT <rfc822;kvm@vger.kernel.org>);
        Thu, 23 Jan 2020 18:33:36 -0500
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
        s=mimecast20190719; t=1579822412;
        h=from:from:reply-to:subject:subject:date:date:message-id:message-id:
         to:to:cc:cc:mime-version:mime-version:content-type:content-type:
         content-transfer-encoding:content-transfer-encoding:
         in-reply-to:in-reply-to:references:references;
        bh=QY7zeZRO22TsoH8HV/WUVhH9pv24i8p9Y+eqVSEOLGg=;
        b=ZOgRGgVXX1qbrYueM0vW/Wa0R2WPC86N8RbGKvW4dnM6Mx7TyCl9fGEMYXqrkl6OrtsDIU
        Oht5YRZH71iW91q/rQyve7WleJOcqtcBUqJqtiK6L4MamDE9A0ulbMZgEce00SO3HPzxe/
        ynBymK6yZO50hp76BFdQFB6R/GlcAvE=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-53-woEsQlmfOLmeiznvHJ5irQ-1; Thu, 23 Jan 2020 18:33:31 -0500
X-MC-Unique: woEsQlmfOLmeiznvHJ5irQ-1
Received: from smtp.corp.redhat.com (int-mx06.intmail.prod.int.phx2.redhat.com [10.5.11.16])
        (using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
        (No client certificate requested)
        by mimecast-mx01.redhat.com (Postfix) with ESMTPS id 5FC2BDB21;
        Thu, 23 Jan 2020 23:33:28 +0000 (UTC)
Received: from w520.home (ovpn-116-28.phx2.redhat.com [10.3.116.28])
        by smtp.corp.redhat.com (Postfix) with ESMTP id 23DB55C290;
        Thu, 23 Jan 2020 23:33:25 +0000 (UTC)
Date:   Thu, 23 Jan 2020 16:33:22 -0700
...
Message-ID: <20200123163322.2376da37@w520.home>
In-Reply-To: <20200121215445.GG1759@joy-OptiPlex-7040>
...
> > Yan Zhao <yan.y.zhao@intel.com> wrote:
> >  =20
> > > On Tue, Jan 21, 2020 at 03:43:02PM +0800, Tian, Kevin wrote: =20
> > > > > From: Alex Williamson <alex.williamson@redhat.com>
...
> > > > > > > > > To: Liu, Yi L <yi.l.liu@intel.com>
> > > > > > > > > Subject: Re: [PATCH v4 11/12] samples: add vfio-mdev-pci =
driver
> > > > > > > > >
> > > > > > > > > On Tue,  7 Jan 2020 20:01:48 +0800
> > > > > > > > > Liu Yi L <yi.l.liu@intel.com> wrote:
> > > > > > > > >   =20
> > > > > > > > > > This patch adds sample driver named vfio-mdev-pci. It i=
s to wrap
> > > > > > > > > > a PCI device as a mediated device. For a pci device, on=
ce bound
> > > > > > > > > > to vfio-mdev-pci driver, user space access of this devi=
ce will
> > > > > > > > > > go through vfio mdev framework. The usage of the device=
 follows
> > > > > > > > > > mdev management method. e.g. user should create a mdev =
before
> > > > > > > > > > exposing the device to user-space.
> > > > > > > > > >
> > > > > > > > > > Benefit of this new driver would be acting as a sample =
driver
> > > > > > > > > > for recent changes from "vfio/mdev: IOMMU aware mediate=
d   =20
> > > > > device"   =20
> > > > > > > > > > patchset. Also it could be a good experiment driver for=
 future
> > > > > > > > > > device specific mdev migration support. This sample dri=
ver only
> > > > > > > > > > supports singleton iommu groups, for non-singleton iomm=
u groups,
> > > > > > > > > > this sample driver doesn't work. It will fail when tryi=
ng to assign
...
> > > > > > > > > >    e.g. use following command to unbind its original dr=
iver   =20
> > > > > > > > > >    > echo $dev_bdf > /sys/bus/pci/devices/$dev_bdf/driv=
er/unbind   =20
> > > > > > > > > >
> > > > > > > > > > c) bind vfio-mdev-pci driver to the physical device   =
=20
> > > > > > > > > >    > echo $vend_id $dev_id > /sys/bus/pci/drivers/vfio-=
mdev-   =20
> > > > > pci/new_id   =20
> > > > > > > > > >
> > > > > > > > > > d) check the supported mdev instances   =20
> > > > > > > > > >    > ls /sys/bus/pci/devices/$dev_bdf/mdev_supported_ty=
pes/   =20
...
name
> > > > > > > > > >
> > > > > > > > > > e)  create mdev on this physical device (only 1 instanc=
e)   =20
...
> > > > > > > > > >     -device vfio-pci,\
> > > > > > > > > >      sysfsdev=3D/sys/bus/mdev/devices/83b8f4f2-509f-382=
f-3c1e-   =20
> > > > > e6bfe0fa1003   =20
> > > > > > > > > >
> > > > > > > > > > g) destroy mdev   =20
> > > > > > > > > >    > echo 1 > /sys/bus/mdev/devices/83b8f4f2-509f-382f-=
3c1e-   =20
> > > > > e6bfe0fa1003/\   =20
> > > > > > > > > >      remove
> > > > > > > > > >
...
> > > > > > > > > >  create mode 100644 samples/vfio-mdev-pci/vfio_mdev_pci=
.c
> > > > > > > > > >
...
> > > > > > > > > > +	tristate "Sample driver for wrapping PCI device as a =
mdev"
> > > > > > > > > > +	select VFIO_PCI_COMMON
> > > > > > > > > > +	select VFIO_PCI
> > > > > > > > > > +	depends on VFIO_MDEV && VFIO_MDEV_DEVICE
> > > > > > > > > > +	help
> > > > > > > > > > +	  Sample driver for wrapping a PCI device as a mdev. =
Once   =20
> > > > > bound to   =20
> > > > > > > > > > +	  this driver, device passthru should through mdev pa=
th.
> > > > > > > > > > +
...
> > > > > > > > >
> > > > > > > > > I think we could just lump this into vfio-mdev rather tha=
n making
...
> > > > > > > > > > diff --git a/samples/vfio-mdev-pci/Makefile b/samples/v=
fio-mdev-   =20
> > > > > pci/Makefile   =20
...
> > > > > > > > > > diff --git a/samples/vfio-mdev-pci/vfio_mdev_pci.c b/sa=
mples/vfio-   =20
> > > > > mdev-   =20
...
> > > > > > > > > > + * Copyright (C) 2012 Red Hat, Inc.  All rights reserv=
ed.
> > > > > > > > > > + *     Author: Alex Williamson <alex.williamson@redhat=
.com>
> > > > > > > > > > + *
> > > > > > > > > > + * Derived from original vfio:
> > > > > > > > > > + * Copyright 2010 Cisco Systems, Inc.  All rights rese=
rved.
...
> > > > > > > > > > +#define DRIVER_DESC     "VFIO Mdev PCI - Sample driver=
 for PCI   =20
> > > > > device as a   =20
> > > > > > > > > mdev"   =20
> > > > > > > > > > +
...
> > > > > > > > > > +MODULE_PARM_DESC(ids, "Initial PCI IDs to add to the v=
fio-mdev-   =20
> > > > > pci driver,   =20
> > > > > > > > > format is   =20
> > > > > \"vendor:device[:subvendor[:subdevice[:class[:class_mask]]]]\" an=
d   =20
> > > > > > > > > multiple comma separated entries can be specified");   =20
> > > > > > > > > > +
> > > > > > > >
> > > > > > > > yes, let me have more study and do better abstract in next =
version. :-)
> > > > > > > >   =20
> > > > > > > > > > +static long vfio_mdev_pci_ioctl(struct mdev_device *md=
ev,   =20
> > > > > unsigned int cmd,   =20
> > > > > > > > > > +			     unsigned long arg)
> > > > > > > > > > +{
...
> > > > > > > > > > +}
> > > > > > > > > > +
> > > > > > > > > > +static ssize_t vfio_mdev_pci_read(struct mdev_device *=
mdev, char   =20
> > > > > __user   =20
> > > > > > > *buf,   =20
...
> > > > > > > > > > +}
> > > > > > > > > > +
> > > > > > > > > > +static ssize_t vfio_mdev_pci_write(struct mdev_device =
*mdev,
...
> > > > > > > > > > +};
> > > > > > > > > > +
> > > > > > > > > > +static int vfio_mdev_pci_driver_probe(struct pci_dev *=
pdev,
...
> > > > > > > > > > +
> > > > > > > > > > +	/*
> > > > > > > > > > +	 * Prevent binding to PFs with VFs enabled, this too =
easily   =20
> > > > > allows   =20
> > > > > > > > > > +	 * userspace instance with VFs and PFs from the same =
device,   =20
> > > > > which   =20
> > > > > > > > > > +	 * cannot work.  Disabling SR-IOV here would initiate=
   =20
> > > > > removing the   =20
> > > > > > > > > > +	 * VFs, which would unbind the driver, which is prone=
 to   =20
> > > > > blocking   =20
> > > > > > > > > > +	 * if that VF is also in use by vfio-pci or vfio-mdev=
-pci. Just
...
=20
> > > > > The only   =20
> > > > > > > > > > +		 * transition it allows from this unknown state is t=
o   =20
> > > > > D0, which   =20
...
> > > > > > > > > > +	}   =20
> > > > > > > > >
> > > > > > > > > Ditto here and remove below, this seems like boilerplate =
that   =20
> > > > > shouldn't   =20
> > > > > > > > > be duplicated per leaf module.  Thanks,   =20
> > > > > > > >
> > > > > > > > Sure, the code snippet above may also be abstracted to be a=
 common   =20
> > > > > API   =20
> > > > > > > > provided by vfio-pci-common.ko. :-)
> > > > > > > >
> > > > > > > > I have a confusion which may need confirm with you. Do you =
also want   =20
> > > > > the   =20
> > > > > > > > below code snippet be placed in the vfio-pci-common.ko and =
exposed   =20
> > > > > out   =20
> > > > > > > > as a wrapped API? Thus it can be used by sample driver and =
other   =20
> > > > > future   =20
...
> > > > > > >
> > > > > > >
> > > > > > > I think some sort of vfio_pci_common_{probe,remove}() would b=
e a
> > > > > > > reasonable starting point where the respective module _{probe=
,remove}
> > > > > > > functions would call into these and add their module specific=
 code
> > > > > > > around it.  That would at least give us a point to cleanup th=
ings that
> > > > > > > are only used by the common code in the common code.   =20
> > > > > >
> > > > > > sure, I can start from here if we are still going with this dir=
ection. :-)
> > > > > >   =20
> > > > > > > I'm still struggling how we make this user consumable should =
we accept
> > > > > > > this and progress beyond a proof of concept sample driver tho=
ugh.  For
> > > > > > > example, if a vendor actually implements an mdev wrapper driv=
er or   =20
> > > > > even   =20
> > > > > > > just a device specific vfio-pci wrapper, to enable for example
> > > > > > > migration support, how does a user know which driver to use f=
or each
> > > > > > > particular feature?  The best I can come up with so far is so=
mething
> > > > > > > like was done for vfio-platform reset modules.  For instance =
a module
> > > > > > > that extends features for a given device in vfio-pci might re=
gister an
> > > > > > > ops structure and id table with vfio-pci, along with creating=
 a module
> > > > > > > alias (or aliases) for the devices it supports.  When a devic=
e is
> > > > > > > probed by vfio-pci it could try to match against registered i=
d tables
> > > > > > > to find a device specific ops structure, if one is not found =
it could
> > > > > > > do a request_module using the PCI vendor and device IDs and s=
ome   =20
> > > > > unique   =20
> > > > > > > vfio-pci string, check again, and use the default ops if devi=
ce
> > > > > > > specific ops are still not present.  That would solve the pro=
blem on
> > > > > > > the vfio-pci side.   =20
> > > > > >
> > > > > > yeah, this is letting vfio-pci to invoke the ops from vendor dr=
ivers/modules.
> > > > > > I think this is what Yan is trying to do.   =20
> > > > >=20
> > > > > I think I'm suggesting a callback ops structure a level above wha=
t Yan
> > > > > previously proposed.  For example, could we have device specific
> > > > > vfio_device_ops where the vendor module can call out to common co=
de
> > > > > rather than requiring common code to test for and optionally call=
 out
> > > > > to device specific code.
> > > > >    =20
> > > > > > > For mdevs, I tend to assume that this vfio-mdev-pci
> > > > > > > meta driver is an anomaly only for the purpose of creating a =
generic
> > > > > > > test device for IOMMU backed mdevs and that "real" mdev vendor
> > > > > > > drivers will just be mdev enlightened host drivers, like i915=
 and
> > > > > > > nvidia are now.  Thanks,   =20
> > > > > >
> > > > > > yes, this vfio-mdev-pci meta driver is just creating a test dev=
ice.
...
> > > > > vfio_device_ops that supports additional features for their devic=
e and
> > > > > we bring that all together with a request module interface and mo=
dule
> > > > > aliases to make that work seamlessly, then it has value.  A conce=
rn I
> > > > > have in only doing this split in order to create the vfio-mdev-pci
> > > > > module is that it leaves open the question and groundwork for for=
king
> > > > > vfio-pci into multiple vendor specific modules that would become =
a mess
> > > > > for user's to mange.
> > > > >    =20
> > > > > > Compared with the "real" mdev vendor drivers, it is like a
> > > > > > "vfio-pci + dummy mdev ops" driver. dummy mdev ops means
> > > > > > no vendor specific handling and passthru to vfio-pci codes dire=
ctly.
> > > > > >
> > > > > > I think this meta driver is even lighter than the "real" mdev v=
endor
> > > > > > drivers. right? Is it possible to let this driver follow the wa=
y of
> > > > > > registering ops structure and id table with vfio-pci? The obsta=
cle
> > > > > > I can see is the meta driver is a generic driver, which means i=
t has
> > > > > > no id table... For the "real" mdev vendor drivers, they natural=
ly have
> > > > > > such info. If vfio-mdev-pci can also get the id info without bi=
nding
> > > > > > to a device, it may be possible. thoughts? :-)   =20
> > > > >=20
> > > > > IDs could be provided via a module option or potentially with
> > > > > build-time options.  That might allow us to test all aspects of t=
he
> > > > > above proposal, ie. allowing sub-modules to provide vfio_device_o=
ps for
> > > > > specific devices, allowing those vendor vfio_device_ops to re-use=
 much
> > > > > of the existing vfio-pci code in that implementation, and a mecha=
nism
> > > > > for generically testing IOMMU backed mdevs.  That's starting to s=
ound a
...
> > > > can be also largely avoided. The vendor driver can directly registe=
r its
...
> > > wrapped into separate modules. so current vfio_pci_register_dev_regio=
n()
...
> > > provide a module named i915_migration to register for regions of inde=
x 0
> > > and 13 for BAR0 and migration. =20
> >=20
