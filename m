...
Received: from out1.vger.email (out1.vger.email [IPv6:2620:137:e000::1:20])
	by mail.lfdr.de (Postfix) with ESMTP id AA18C7A5794
	for <lists+kvm@lfdr.de>; Tue, 19 Sep 2023 04:57:29 +0200 (CEST)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S231217AbjISC5d (ORCPT <rfc822;lists+kvm@lfdr.de>);
        Mon, 18 Sep 2023 22:57:33 -0400
Received: from lindbergh.monkeyblade.net ([23.128.96.19]:39928 "EHLO
        lindbergh.monkeyblade.net" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S231190AbjISC5b (ORCPT <rfc822;kvm@vger.kernel.org>);
        Mon, 18 Sep 2023 22:57:31 -0400
Received: from smtp.kernel.org (relay.kernel.org [52.25.139.140])
        by lindbergh.monkeyblade.net (Postfix) with ESMTPS id D889610E;
        Mon, 18 Sep 2023 19:57:24 -0700 (PDT)
Received: by smtp.kernel.org (Postfix) with ESMTPSA id 60900C433CB;
        Tue, 19 Sep 2023 02:57:24 +0000 (UTC)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/simple; d=kernel.org;
        s=k20201202; t=1695092244;
        bh=qo6n8cA/KiwvnrKZ5aIeajMWB4+n4tqFxJU2jjU0b88=;
        h=References:In-Reply-To:From:Date:Subject:To:Cc:From;
        b=
Received: by mail-ed1-f48.google.com with SMTP id 4fb4d7f45d1cf-532aaba8747so759540a12.3;
        Mon, 18 Sep 2023 19:57:24 -0700 (PDT)
X-Gm-Message-State: AOJu0YxZnkJeGtbs7V0r6L4GUCypptsysIdR8qvM3bQDluvLq++HLC7m
        28hvSM0wTyONCN5CD4Otah77g5f+Vz4KEzFAC2k=
X-Google-Smtp-Source: AGHT+IGB+H/7tH+qqzS2BUYr7j1pnd0m5etkzR2tvjBR8JVVr9oNnjFCDtpw+tNW3fUAkk4cywVjpw3hHQ3pQL7uiIo=
X-Received: by 2002:aa7:cf94:0:b0:52c:8a13:2126 with SMTP id
 z20-20020aa7cf94000000b0052c8a132126mr10055343edx.37.1695092242673; Mon, 18
 Sep 2023 19:57:22 -0700 (PDT)
MIME-Version: 1.0
References: <20230915014949.1222777-1-zhaotianrui@loongson.cn>
 ...
 <CAAhV-H49YTLJKW=rnp_=GOtGby-QG_S=bwQSFy+MWgkFRmUjCg@mail.gmail.com> <35658917-c7d6-19ad-ea23-848a4ef00a97@loongson.cn>
In-Reply-To: <35658917-c7d6-19ad-ea23-848a4ef00a97@loongson.cn>
From:   Huacai Chen <chenhuacai@kernel.org>
Date:   Tue, 19 Sep 2023 10:57:10 +0800
X-Gmail-Original-Message-ID: <CAAhV-H7xAkY5qe55=Ftn0qOaUNKGwrLcPJ84KbX9EiqD1T5fmA@mail.gmail.com>
Message-ID: <CAAhV-H7xAkY5qe55=Ftn0qOaUNKGwrLcPJ84KbX9EiqD1T5fmA@mail.gmail.com>
Subject: Re: [PATCH v21 02/29] LoongArch: KVM: Implement kvm module related interface
To:     zhaotianrui <zhaotianrui@loongson.cn>
Cc:     bibo mao <maobibo@loongson.cn>, linux-kernel@vger.kernel.org,
...
On Tue, Sep 19, 2023 at 10:53=E2=80=AFAM zhaotianrui <zhaotianrui@loongson.=
cn> wrote:
>
>
> =E5=9C=A8 2023/9/19 =E4=B8=8A=E5=8D=8810:14, Huacai Chen =E5=86=99=E9=81=
=93:
> > On Mon, Sep 18, 2023 at 4:39=E2=80=AFPM bibo mao <maobibo@loongson.cn> =
wrote:
> >>
> >>
...
=81=93:
> >>>> On Mon, Sep 18, 2023 at 11:20=E2=80=AFAM zhaotianrui <zhaotianrui@lo=
ongson.cn> wrote:
> >>>>> =E5=9C=A8 2023/9/18 =E4=B8=8A=E5=8D=8811:12, Huacai Chen =E5=86=99=
=E9=81=93:
> >>>>>> On Mon, Sep 18, 2023 at 11:08=E2=80=AFAM zhaotianrui <zhaotianrui@=
loongson.cn> wrote:
> >>>>>>> =E5=9C=A8 2023/9/18 =E4=B8=8A=E5=8D=889:45, Huacai Chen =E5=86=99=
=E9=81=93:
> >>>>>>>> On Mon, Sep 18, 2023 at 9:21=E2=80=AFAM zhaotianrui <zhaotianrui=
@loongson.cn> wrote:
> >>>>>>>>> =E5=9C=A8 2023/9/16 =E4=B8=8B=E5=8D=884:51, Huacai Chen =E5=86=
=99=E9=81=93:
> >>>>>>>>>> Hi, Tianrui,
> >>>>>>>>>>
> >>>>>>>>>> On Fri, Sep 15, 2023 at 9:50=E2=80=AFAM Tianrui Zhao <zhaotian=
rui@loongson.cn> wrote:
...
> >>>>>>>>>>>
> >>>>>>>>>>> diff --git a/arch/loongarch/kvm/main.c b/arch/loongarch/kvm/m=
ain.c
...
> >>>>>>>>>>> +/*
> >>>>>>>>>>> + * Copyright (C) 2020-2023 Loongson Technology Corporation L=
imited
> >>>>>>>>>>> + */
> >>>>>>>>>>> +
> >>>>>>>>>>> +#include <linux/err.h>
...
> >>>>>>>>>>> + * function to set the flag to 1(SW_GCSR) or 2(HW_GCSR) if t=
he
> >>>>>>>>>>> + * gcsr is software or hardware. It will be used by get/set_=
gcsr,
> >>>>>>>>>>> + * if gcsr_flag is HW we should use gcsrrd/gcsrwr to access =
it,
...
> >>>>>>>>>>
> >>>>>>>>>> Huacai
> >>>>>>>>> The FWPC and MWPC should be SW GCSR, FWPS and MWPS should be HW=
 GCSR, it
> >>>>>>>>> is my mistake.
> >>>>>>>> But in user manual vol 3, section 1.5, FWPC/FWPS/MWPC/MWPS are a=
ll HW
> >>>>>>>> GCSR, while DBxxxx and IBxxxx are SW GCSR.
> >>>>>>> Ok, It is my misunderstanding, as the FWPC and MWPC can control g=
uest
> >>>>>>> debug register numbers, but I know they are all HW GCSR when I lo=
ok up
> >>>>>>> the manual again.
...
> >>> If guest can access hw CSR directly and do not cause a exception, it =
means hw GCSR. on the other hand, if cause a exception to return to KVM to =
emulate it by software, it means SW GCSR. And I re-check the PGD, TINTCLR, =
PRCFG1~3 CSR, they should be hw GCSR.
> >> CSR_PGD is logical read only register instead, its content is the same
> >> CSR_PGDL if highest bit of BADV is 0, else its content is the same wit=
h
> >> CSR_PGDH.
> >>
> >> TINTCLR is write only register, write 1 to clear timer interrupt and r=
ead
> >> value is 0.
> >>
> >> So CSR_PGD and TINTCLR need not save and restore during vcpu switching=
 or
> >> vm migration, it is not necessary to set CSR_PGD/TINTCLR from qemu use=
r space.
> >>
> >> PRCFG1~3 is read only registers, it is decided by hardware and can not=
 be
> >> set by kvm, it is not necessary to save or retore. However there maybe=
 brings
> >> problems for vm migration on diferent hardwares if PRCFG1~3 is differe=
nt.
...
> we will add this handling later when we want to use this function in gues=
t.
...
> >>>>>>>>>>> +       /*
> >>>>>>>>>>> +        * Are we entering guest context on a different CPU t=
o last time?
> >>>>>>>>>>> +        * If so, the vCPU's guest TLB state on this CPU may =
be stale.
> >>>>>>>>>>> +        */
> >>>>>>>>>>> +       context =3D per_cpu_ptr(vcpu->kvm->arch.vmcs, cpu);
...
> >>>>>>>>>>> +        *
> >>>>>>>>>>> +        * We also discard the stored vpid if we've executed =
on
> >>>>>>>>>>> +        * another CPU, as the guest mappings may have change=
d without
...
> >>>>>>>>>>> +       vpid =3D (vcpu->arch.vpid & vpid_mask) << CSR_GSTAT_G=
ID_SHIFT;
> >>>>>>>>>>> +       change_csr_gstat(vpid_mask << CSR_GSTAT_GID_SHIFT, vp=
id);
> >>>>>>>>>>> +}
> >>>>>>>>>>> +
...
> >>>>>>>>>>> +       if (!vmcs) {
> >>>>>>>>>>> +               pr_err("kvm: failed to allocate percpu kvm_co=
ntext\n");
> >>>>>>>>>>> +               return -ENOMEM;
> >>>>>>>>>>> +       }
> >>>>>>>>>>> +
> >>>>>>>>>>> +       kvm_loongarch_ops =3D kzalloc(sizeof(*kvm_loongarch_o=
ps), GFP_KERNEL);
...
> >>>>>>>>>>> +       }
> >>>>>>>>>>> +       /*
> >>>>>>>>>>> +        * There will be problem in world switch code if ther=
e
> >>>>>>>>>>> +        * is page fault reenter, since pgd register is share=
d
> >>>>>>>>>>> +        * between root kernel and kvm hypervisor. World swit=
ch
> >>>>>>>>>>> +        * entry need be unmapped area, cannot be tlb mapped =
area.
> >>>>>>>>>>> +        * In future if hw pagetable walking is supported, or=
 there
> >>>>>>>>>>> +        * is separate pgd registers between root kernel and =
kvm
> >>>>>>>>>>> +        * hypervisor, copying about world switch code will n=
ot be used.
> >>>>>>>>>>> +        */
> >>>>>>>>>>> +
> >>>>>>>>>>> +       order =3D get_order(kvm_vector_size + kvm_enter_guest=
_size);
...
> >>>>>>>>>>> +       memcpy(addr + kvm_vector_size, kvm_enter_guest, kvm_e=
nter_guest_size);
> >>>>>>>>>>> +       flush_icache_range((unsigned long)addr, (unsigned lon=
g)addr +
> >>>>>>>>>>> +                               kvm_vector_size + kvm_enter_g=
uest_size);
> >>>>>>>>>>> +       kvm_loongarch_ops->guest_eentry =3D addr;
> >>>>>>>>>>> +       kvm_loongarch_ops->enter_guest =3D addr + kvm_vector_=
size;
> >>>>>>>>>>> +       kvm_loongarch_ops->page_order =3D order;
> >>>>>>>>>>> +
> >>>>>>>>>>> +       vpid_mask =3D read_csr_gstat();
> >>>>>>>>>>> +       vpid_mask =3D (vpid_mask & CSR_GSTAT_GIDBIT) >> CSR_G=
STAT_GIDBIT_SHIFT;
> >>>>>>>>>>> +       if (vpid_mask)
...
> >>>>>>>>>>> +                       addr =3D (unsigned long)kvm_loongarch=
_ops->guest_eentry;
> >>>>>>>>>>> +                       free_pages(addr, kvm_loongarch_ops->p=
age_order);
> >>>>>>>>>>> +               }
> >>>>>>>>>>> +               kfree(kvm_loongarch_ops);
> >>>>>>>>>>> +       }
...
> >>>>>>>>>>> +
> >>>>>>>>>>> +       if (!cpu_has_lvz) {
> >>>>>>>>>>> +               kvm_info("hardware virtualization not availab=
le\n");
> >>>>>>>>>>> +               return -ENODEV;
> >>>>>>>>>>> +       }
...
> >>>>>>>>>>> +               return r;
> >>>>>>>>>>> +
> >>>>>>>>>>> +       return kvm_init(sizeof(struct kvm_vcpu), 0, THIS_MODU=
LE);
> >>>>>>>>>>> +}
> >>>>>>>>>>> +
